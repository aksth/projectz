  ...
  get(existingEmailRef)
    .then((snapshot) => {
      if (snapshot.exists()) {
        ...
        update(reference, recipeToAdd).then(() => {
...
function deleteMealPlanItem(email, recipeId, callback) {
  const db = getDatabase();
  const existingEmailRef = query(
    ref(db, 'users'),
    orderByChild('email'),
  ...
  get(existingEmailRef)
    .then((snapshot) => {
      if (snapshot.exists()) {
        console.log('user by email exists!');
        console.log('Deleting meal plan item for ' + email);
        const id = Object.keys(snapshot.val())[0];
        const reference = ref(db, 'users/' + id + '/mealplan/' + recipeId);
        remove(reference).then(() => {
          Toast.show(`Deleted from the meal plan!`, {
            duration: Toast.durations.SHORT,
            animation: true,
            hideOnPress: true,
          });
